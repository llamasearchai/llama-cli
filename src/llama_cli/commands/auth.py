...
def login(
    ...
    try:
        ...
        print_success(
            f"Successfully logged in to LlamaSearch.ai services with profile '{active_profile}'"
        )
...
def status(
    ...
    try:
        ...
        if api_key:
            ...
            console.print(
                f"API URL: [bold]{config.get('profiles', {}).get(active_profile, {}).get('api_url', 'https://api.llamasearch.ai')}[/]"
            )
...
def list_profiles():
    ...
    for profile_name, profile_data in profiles.items():
        ...
        console.print(
            f"API URL: [bold]{profile_data.get('api_url', 'https://api.llamasearch.ai')}[/]"
        )
