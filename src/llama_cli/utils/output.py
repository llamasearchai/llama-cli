    ...
    if output_format == "json":
        ...
        if headers is None:
            if isinstance(data[0], dict):
                ...
                headers = [
                    f"Column{i+1}"
                    for i in range(len(data[0]) if hasattr(data[0], "__len__") else 1)
                ]
...
def print_output(
    ...
    if output_format == "json":
        console.print(
            Syntax(json.dumps(data, indent=2, default=str), "json", theme="monokai")
        )

    elif output_format == "yaml":
        console.print(
            Syntax(yaml.dump(data, default_flow_style=False), "yaml", theme="monokai")
        )
